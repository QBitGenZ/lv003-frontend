export const ProductData = [
    {
        ProductNo: "001",
        ProductName: "SKIN 1004",
        ProductImage: process.env.PUBLIC_URL + "/images/client/product001.png",
        ProductDescription:
            "SKIN1004 Madagascar Centella Hyalu-Cica Water-Fit Sun Serum 50ml",
        ProductVote: "4.6",
        ProductPrice: "350.000vnd",
    },
    {
        ProductNo: "002",
        ProductName: "Instree",
        ProductImage: process.env.PUBLIC_URL + "/images/client/product002.png",
        ProductDescription:
            "Isntree Hyaluronic Acid Watery Sun Gel Double Set (50mL+50mL)",
        ProductVote: "4.2",
        ProductPrice: "350.000vnd",
    },
    {
        ProductNo: "003",
        ProductName: "AESTURA",
        ProductImage: process.env.PUBLIC_URL + "/images/client/product003.png",
        ProductDescription: "AESTURA Atobarrier 365 Cream 80mL Double Set",
        ProductVote: "4.6",
        ProductPrice: "350.000vnd",
    },
    {
        ProductNo: "004",
        ProductName: "Real Barrier",
        ProductImage: process.env.PUBLIC_URL + "/images/client/product004.png",
        ProductDescription: "Real Barrier Cream Cleaning Foam 120ml Duo Pack",
        ProductVote: "4.6",
        ProductPrice: "350.000vnd",
    },
    {
        ProductNo: "005",
        ProductName: "ROUND LAB",
        ProductImage: process.env.PUBLIC_URL + "/images/client/product005.png",
        ProductDescription:
            "ROUND LAB Birch Juice Moisturizing Ampoule 50ml Special Set (+Cream 20ml)",
        ProductVote: "4.6",
        ProductPrice: "350.000vnd",
    },
];
